...
const DashboardPage: React.FC = () => {
  ...
  // ⭐ One-line RBAC (future‑proof)
  const rbac = useRBAC();
  /// ⭐ Unified RBAC flags (future‑proof)
 ...
 // ⭐ FIX #1 — Wait for authentication before fetching assets
  useEffect(() => {
 ...
 // ⭐ Unified dashboard fetcher (used for refresh)
  const refreshDashboard = () => {
    axiosInstance
      .get("/dashboard")
      .then((res) => {
        setMetrics(res.data.metrics || { total: 0, completed: 0, open: 0, risk: "N/A" });
 ...
 // ⭐ FIX #3 — Prevent UI from rendering before token loads
  if (!isAuthenticated) {
  ...
  return (
    <div
      className="min-h-screen flex bg-gray-100 m-0 p-0">
      {/* Sidebar navigation */}
      <div className="w-64 min-h-screen p-6  space-y-4 bg-[#0989B1] text-white">
        ...
        <button
          ...
          style={{
          backgroundColor: activeSection === "metrics" ? "#549E39" : "transparent",
        ...
        <button
          ...
          className="block w-full text-left px-2 py-1 rounded font-medium hover:opacity-80"
          style={{
          backgroundColor: activeSection === "priorities" ? "#549E39" : "transparent",
        ...
        <button
          ...
          className="block w-full text-left px-2 py-1 rounded font-medium hover:opacity-80"
           style={{
           backgroundColor: activeSection === "mapping" ? "#549E39" : "transparent",
      ...
      <div className="flex-1 max-w-6xl mx-auto grid grid-cols-1 gap-6">
        ...
        {activeSection === "priorities" && (
          <TopPriorities
            ...
            refreshDashboard={refreshDashboard} // ⭐ IMPORTANT
          />
        )}
