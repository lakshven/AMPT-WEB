...
export default async function getStaticOptions(req: Request, res: Response): Promise<void> {
  try {
    const range = (n: number) => Array.from({ length: n + 1 }, (_, i) => i.toString());
    const range1to = (n: number) => Array.from({ length: n }, (_, i) => `${i + 1}`);
