...
function resolveCompany(req: Request, res: Response): number | "ALL" | null {
  const role = String(req.user?.role);
  const companyId = req.user?.companyId;

  if (role === "app_admin") return "ALL";
  if(role=== "single_user") return null;
...
export const createIssue = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const newIssue = await prisma.assetIssue.create({
      data: {
        assetId,
        code,
        title,
...
export const getIssueById = async (req: Request, res: Response) => {
  ...
  try {
    const issue = await prisma.assetIssue.findUnique({ where: { id: issueId } });
...
export const updateIssue = async (req: Request, res: Response) => {
  ...
  try {
    const existing = await prisma.assetIssue.findUnique({ where: { id: issueId } });
...
// COMPLETE ISSUE
export const completeIssue = async (req: Request, res: Response) => {
  const company = resolveCompany(req, res);
  if (company == null) return;
