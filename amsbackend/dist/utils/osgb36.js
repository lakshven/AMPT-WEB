...
function osgb36ToWgs84(input) {
    ...
    while (true) {
        ...
        const Mb = (3 * n + 3 * n * n + (21 / 8) * n * n * n) *
            Math.sin(lat - lat0) *
            Math.cos(lat + lat0);
        const Mc = ((15 / 8) * n * n + (15 / 8) * n * n * n) *
            Math.sin(2 * (lat - lat0)) *
            Math.cos(2 * (lat + lat0));
        const Md = (35 / 24) *
            n *
...
function helmertTransform(lat, lon) {
    const H = 0;
    const a = 6377563.396;
    const b = 6356256.909;
    const e2 = (a * a - b * b) / (a * a);
...
//# sourceMappingURL=osgb36.js.map